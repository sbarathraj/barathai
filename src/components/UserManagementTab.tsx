...
const UserManagementTab: React.FC<UserManagementTabProps> = ({
  ...
  const exportToExcel = async () => {
    ...
    XLSX.writeFile(
      wb,
      `users${dateRange}_${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.xlsx`,
    );
  };
  ...
  return (
    <div className="space-y-4">
      {toast && (
        <div
          className={`fixed top-6 left-1/2 -translate-x-1/2 px-6 py-3 rounded-lg shadow-lg text-white text-sm font-medium transition-all z-50 ${toast.type === "success" ? "bg-green-500" : "bg-red-500"}`}
        >
          {toast.message}
        </div>
      )}
      <div className="bg-white/90 dark:bg-slate-900/90 rounded-2xl p-4 sm:p-8 shadow-xl border border-white/30 dark:border-slate-800/40 backdrop-blur-xl overflow-x-auto">
        <h3 className="text-xl font-bold mb-4 text-slate-800 dark:text-slate-100 bg-gradient-to-r from-blue-600 to-pink-500 bg-clip-text text-transparent">
          User Management
        </h3>
        {loading ? (
                ...
                <TableBody>
                  {users
                    ...
                    .map((user, index) => (
                      <TableRow
                        key={user.id}
                        className="hover:bg-blue-50/40 dark:hover:bg-pink-900/10 transition-colors"
                      >
                        <TableCell>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <div className="p-6 max-w-md w-full mx-auto bg-white dark:bg-slate-900 rounded-2xl shadow-xl flex flex-col gap-4">
            ...
            <input
              className="border border-blue-300 focus:border-blue-500 rounded px-3 py-2 text-sm outline-none bg-white/80 dark:bg-slate-800/80"
              value={editName}
